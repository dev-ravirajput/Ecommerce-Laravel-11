    ...
    <section class="product-single container">
      <div class="row">
                ...
                <div class="swiper-wrapper">
                  <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
                      ...
                      @if (!empty($product->images))
						    @foreach (explode(',', $product->images ?? '') as $img)
                  <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
                      src="{{ asset('uploads/products/thumbnails/'. $img) }}" width="104" height="104" alt="" /></div>
                      @endforeach
						@endif
                </div>
              </div>
        ...
        <div class="col-lg-5">
          ...
          @else
          <form action="{{ route('add.to.wishlist') }}" method="POST" id="wishlist-form">
                  @csrf
                  <input type="hidden" name="id" value="{{ $product->id }}">
                  <input type="hidden" name="name" value="{{ $product->name }}">
              ...
              <div class="pc__img-wrapper">
                <a href="{{ route('product.details', $rproduct->slug) }}">
                  <img loading="lazy" src="{{ asset('uploads/products/'.$rproduct->image) }}" width="330" height="400"
                    ...
                    @foreach (explode(',', $rproduct->images ?? '') as $rimg)
                  <img loading="lazy" src="{{ asset('uploads/products/'.$rimg) }}" width="330" height="400"
